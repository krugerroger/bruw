// app/[locale]/api/submit-booking/route.ts
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const additionalMessage =
      formData.get("additionalMessage")?.toString() || "";
    ...
    if (!fileExt) {
      return NextResponse.json(
        {
          error:
            "Type de fichier non autorisé (seuls JPG, PNG, PDF sont acceptés)",
        },
        { status: 400 },
      );
    }

    // ✅ APRÈS (Nom de fichier sécurisé)

    // 1. On sépare le nom de base sans l'extension
    const baseName = paymentProof.name.substring(
      0,
      paymentProof.name.lastIndexOf("."),
    );

    // 2. On nettoie le nom : remplace les espaces, accents et caractères spéciaux par des tirets
    const sanitizedBaseName = baseName
      .normalize("NFD") // Décompose les caractères accentués (é -> e)
      .replace(/[\u0300-\u036f]/g, "") // Supprime les diacritiques
      .replace(/[^a-zA-Z0-9_-]/g, "_"); // Remplace tout le reste par des "_"

    // 3. Recomposition du nom propre
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("bookings").getPublicUrl(filePath);
