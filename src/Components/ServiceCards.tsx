...
function useServices(): Offer[] {
  const t = useTranslations("ServiceCards.services.intimateMoment");
  ...
  return [
    {
      ...
      price: t.raw("options") as OfferOption[],
    },
  ];
}
...
const ServiceCards = () => {
  ...
  const handleSelect = (data: Offer) => {
    router.push(
      `/reservation?data=${encodeURIComponent(JSON.stringify(data))}`,
    );
  };
        ...
        >
          <div className="card-body p-4 md:p-6">
            <div className="flex justify-between items-start">
              <h3 className="card-title text-amber-600 text-xl md:text-2xl">
                {service.title}
              </h3>
            </div>
            ...
            <div className="bg-stone-900/80 rounded-box p-3 md:p-4">
              <h4 className="font-bold text-amber-600 mb-6 text-base md:text-lg">
                {t("availableOptions")}
              </h4>
              <ul className="space-y-3">
                {service.price.map((option, idx) => (
                  <li
                    key={idx}
                    className="bg-gray-800/40 rounded-lg p-3 hover:bg-gray-700/60 transition-colors"
                  >
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-1 md:gap-0">
                      ...
                      <div
                        className="text-gray-300 md:text-right text-sm md:text-base max-w-xs mt-1 md:mt-0"
            ...
            <div className="card-actions justify-end mt-6">
              <button
                ...
                className="btn btn-primary bg-amber-600 hover:bg-amber-700 border-none w-full md:w-auto"
              >
                {t("bookButton")}
              </button>
            </div>
